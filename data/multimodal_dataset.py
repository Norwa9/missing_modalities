...
if __name__ == '__main__':
    ...
    x = [a[100], a[34], a[890]] # 从dataloader取3个样本
    print('each one:')
    for i, _x in enumerate(x):
        ...
        for k, v in _x.items():
            if k not in ['int2name', 'label']:
                ...
                print(k, v)
    '''
    从dataloader取3个样本，发现长度的维度不一致

    0 :
    A_feat torch.Size([43, 130])
    V_feat torch.Size([50, 342])
    L_feat torch.Size([22, 1024])
    label tensor(1)
    int2name Ses02M_impro07_M020
    1 :
    A_feat torch.Size([62, 130])
    V_feat torch.Size([50, 342])
    L_feat torch.Size([22, 1024])
    label tensor(2)
    int2name Ses02M_script02_2_M017
    2 :
    A_feat torch.Size([26, 130])
    V_feat torch.Size([50, 342])
    L_feat torch.Size([22, 1024])
    '''


    # 3个样本的维度都不一致，使用collate_fn来进行padding
    print('packed output')
    x = a.collate_fn(x) 
    for k, v in x.items():
        if k not in ['int2name', 'label']:
    ...
    '''
    经过collate_fn，一个batch(size=3)中A、V、L都padding到了它们当中最长的那个序列的长度
    A_feat torch.Size([3, 62, 130])
    V_feat torch.Size([3, 50, 342])
    L_feat torch.Size([3, 22, 1024])
    '''