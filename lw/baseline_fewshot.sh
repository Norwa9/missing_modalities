...
cmd="python train_baseline.py --dataset_mode=multimodal_fewshot --model=utt_fusion
...
--name=baseline_fewshot --suffix={modality}_run{run_idx} --has_test
--cvNo=$i"
...
# 使用图2(b)的简单模型训练的baseline，效果就已经超过了sota了（？）
# 这里我用fewshot数据集替换了原有的数据集，其他不变
# bash lw/baseline_fewshot.sh AVL 1 4