...
# from models.networks.CLIP_model import clip_model
...
class UttFusionModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # CLIP by 罗威
        # if 'L' in self.modality and 'V' in self.modality:
            # self.model_names.append('L')
            # self.model_names.append('V')
            # self.netVL = clip_model(opt.embd_size_v, opt.embd_size_l) # CLIP

            
        if self.isTrain:
            self.criterion_ce = torch.nn.CrossEntropyLoss()
            # initialize optimizers; schedulers will be automatically created by function <BaseModel.setup>.
            paremeters = [{'params': getattr(self, 'net'+net).parameters()} for net in self.model_names] # 只更新model_names里面指定的参数
    ...
    def forward(self):
        ...
        # if 'L' in self.modality and 'V' in self.modality:
        #     self.feat_VL = self.netVL(self.visual, self.lexical) # clip，将image和text编码后相接
        #     final_embd.append(self.feat_VL)
